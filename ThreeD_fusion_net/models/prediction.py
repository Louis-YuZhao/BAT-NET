...
def run_validation_case(test_index, out_dir, model_file, hdf5_file, validation_keys_file, training_modalities,
    ...
    if isinstance(prediction_image, list):
        for i, image in enumerate(prediction_image):
            image.to_filename(os.path.join(out_dir, "PID_{0}_prediction_{1}.nii.gz".format(data_index, i + 1)))
    else:
        prediction_image.to_filename(os.path.join(out_dir, "PID_{0}_prediction.nii.gz".format(data_index)))

    data_file.close()

def trans_validation_case(test_index, out_dir, hdf5_file, validation_keys_file, training_modalities):
    """
    Runs a test case and writes predicted images to file.
