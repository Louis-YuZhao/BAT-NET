...
def write_data_to_file(list_of_data_files, output_file, image_shape, truth_dtype=np.uint8, ifUSENorm = True):
    """
...
def get_discriminator_input_and_label(combineNet_output, combineNet_label):
    D_label_idx = np.random.choice(len(combineNet_label), len(combineNet_label)//2, replace=False)
    D_input = combineNet_output
    D_input[D_label_idx] = combineNet_label[:,np.newaxis,:,:,:][D_label_idx]
    D_label = np.zeros((len(combineNet_output),1))
